...
- Remembered credential sessions use a longer JWT lifetime; unchecked sessions use a shorter browser-session-style JWT lifetime.
...
- The app has PWA install metadata:
  - `src/app/manifest.ts`
  - PNG app icons under `public/icons`
  - Apple web app metadata
  - `public/sw.js` service worker registered by `PwaRegistrar`
- PWA service worker registration is production-only; local dev unregisters existing service workers so Turbopack updates are not hidden by cached app shell behavior.
...
- `/manifest.webmanifest` is generated by Next.js and included in the production build for installable app support.
